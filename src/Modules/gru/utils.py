...
def load_train_data(path, word2idx=None, max_seq_len=None, label=True, preprocessing=True):
    if label:
...
def load_test_data(path, word2idx=None, max_seq_len=None, preprocessing=True):
    with open(path, 'r') as f:
        f_all = [ll[ll.find(',') + 1:] for ll in f.readlines()[1:]]
    test = [text_to_word_sequence(ll) for ll in f_all]
    return data_preprocessing(test, word2idx, max_seq_len) if preprocessing else test  # split the string to list of words
