...
class Doc2VecC(BaseEstimator):
    ...
    def prepare_documents(documents, filename, replace=False):
        ...
        with open(filename, 'w') as f:
            tokenized = False#util.is_tokenized(documents)
    ...
    def generate_vocabulary(self, documents, replace=False):
        ...
        :return:
        """
        with EventTimer('Preparing documents'):
        ...
        if replace or not os.path.exists(self.vocab_file):
            ...
            with EventTimer('c_doc2vecc.generate_vocab_file'):
        ...
        with EventTimer('Training model'):
            if not self.keep_generated_files:
                ...
                self.input_weights, self.docvecs = c_doc2vecc.train(size=self.size,
                                                                    ...
                                                                    negative=self.negative,
                                                                    num_threads=self.threads,
                                                                    cbow=self.cbow, binary=self.binary,
                                                                    iter=self.epochs,
                                                                    rp_sample=self.sentence_sample,
                                                                    alpha=self.alpha,
                                                                    read_vocab=self.vocab_file,
    ...
    def embed(self, X):
        ...
        self.prepare_documents(X, tmp_transform_file, replace=True)
    ...
    def infer_vector(self, text):
        """
        :param text: query string or list of query strings
        :return: embedding vector or numpy array of embedding vectors
        """
